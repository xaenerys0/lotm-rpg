...
- `/api/proxy/ollama-cloud/api/embed` — POST proxy for RAG **query embedding** (issue #60, hosted cloud embedding). Forwards the Ollama-native `/api/embed` body (`{model, input}`) to `https://ollama.com/api/embed`. Two differences from the sibling proxies: (1) it's **auth-gated** — `createClient()` from `@/lib/supabase/server` → `getUser()`, 401 if signed out; (2) unlike the chat proxy (which forwards the _caller's_ BYOK key), this **injects the operator's** server-held `OLLAMA_CLOUD_API_KEY` (never `NEXT_PUBLIC_`; 503 if unset) so every signed-in player gets retrieval and the browser sends no key. The operator pays per query, so it is also **per-user rate-limited**: after the auth gate it calls the `check_rate_limit` SECURITY DEFINER RPC (bucket `"embed"`) and returns **429 + Retry-After** when a user exceeds `EMBED_RATE_LIMIT_MAX` requests per `EMBED_RATE_LIMIT_WINDOW_SECONDS` (defaults 30/60s; `EMBED_RATE_LIMIT_MAX=0` disables). Fails **closed** (503) if the limit can't be verified — it must not spend the key unchecked; `retrieveLoreForTurn` swallows both 429 and 503 best-effort, so a throttled turn just runs on curated lore. Logs upstream ≥400 status + requested model to Vercel logs; the key is never logged. Used by the `ollama-cloud` embedding transport (`@/lib/ai` `embeddings.ts`) via `lore-retrieval-client.ts` when `NEXT_PUBLIC_OLLAMA_CLOUD_EMBEDDING` is set.
