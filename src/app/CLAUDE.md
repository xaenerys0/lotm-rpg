...
- `/api/proxy/cloudflare/embed` — POST proxy for RAG **query embedding** (issue #60, hosted cloud embedding via Cloudflare Workers AI). Forwards the `{text:[…]}` body to `https://api.cloudflare.com/client/v4/accounts/<CF_ACCOUNT_ID>/ai/run/@cf/baai/bge-m3` and relays Cloudflare's `{result:{data}}` response. (1) **Auth-gated** — `createClient()` from `@/lib/supabase/server` → `getUser()`, 401 if signed out; (2) **injects the operator's** server-held `CF_ACCOUNT_ID` + `CF_API_TOKEN` (never `NEXT_PUBLIC_`; 503 if unset) so every signed-in player gets retrieval and the browser sends no key. The operator spends a free-tier quota, so it is also **per-user rate-limited**: after the auth gate it calls the `check_rate_limit` SECURITY DEFINER RPC (bucket `"embed"`) and returns **429 + Retry-After** when a user exceeds `EMBED_RATE_LIMIT_MAX` requests per `EMBED_RATE_LIMIT_WINDOW_SECONDS` (defaults 30/60s; `EMBED_RATE_LIMIT_MAX=0` disables). Fails **closed** (503) if the limit can't be verified; `retrieveLoreForTurn` swallows both 429 and 503 best-effort, so a throttled turn just runs on curated lore. Logs upstream ≥400 status to Vercel logs; the token/request text are never logged. Used by the `cloudflare` embedding transport (`@/lib/ai` `embeddings.ts`) via `lore-retrieval-client.ts` when `NEXT_PUBLIC_CLOUDFLARE_EMBEDDING` is set. (Replaced an earlier `/api/proxy/ollama-cloud/api/embed` route: ollama.com hosts no embedding models, so its `/api/embed` 401s.)
