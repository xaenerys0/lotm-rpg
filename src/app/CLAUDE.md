...
- `/api/proxy/ollama-cloud/chat/completions` — Server-side proxy for ollama.com. The cloud service does not send CORS headers, so direct browser fetches fail with status 0. This route forwards the OpenAI-compatible request body to `https://ollama.com/v1/chat/completions` with the caller's Bearer key and relays the response. **It streams the upstream body straight through** (`upstream.body.pipeThrough(new TransformStream(...))`) instead of buffering the whole generation before the first byte — the buffered path made TTFB equal the full ~18s generation time; a bodyless upstream falls back to buffering. Sets an explicit `export const maxDuration = 60` (a slow generation streams to completion instead of hitting a shorter platform default and 504-ing) and `export const runtime = "nodejs"` (streaming + the tee-and-log diagnostic below need Node, not Edge). The diagnostic runs in the passthrough's `flush` (once the stream completes): on a non-2xx status it `console.error`s the status, requested model, and (truncated) upstream body; on a **2xx** it runs `summarizeUnparseableChat` and logs a structured diagnostic **only** when the body doesn't look like the game's JSON (reasoning models like gpt-oss/Harmony can pollute `content`, put reasoning in a separate `reasoning`/`thinking` field, or truncate via `finish_reason: "length"`) — capturing the finish reason, content length/empty flag, whether a `"narrative"` key is present, the reasoning field/length, and head/tail snippets of the AI's own output. The `Authorization` header/key and the player's request text are never logged. Used exclusively by `OllamaCloudAdapter`. (The player's per-model reasoning depth rides on the request body as `reasoning_effort` — see `@/lib/ai` `thinking.ts` — and passes through this proxy untouched.)
- `/api/proxy/ollama-cloud/models` — Companion GET proxy that forwards to `https://ollama.com/v1/models` (same CORS workaround) so `OllamaCloudAdapter.listModels()` can populate the model dropdown.
  - NOTE: there is intentionally **no** `images/generations` proxy. Ollama Cloud is not an image backend — ollama.com hosts only chat/vision models and its image-generation models (z-image / flux2-klein) run locally only, so scene art uses local Ollama / OpenAI / local Stable Diffusion instead (`@/lib/ai/image-providers`).
- `/api/proxy/cloudflare/embed` — POST proxy for RAG **query embedding** (issue #60, hosted cloud embedding via Cloudflare Workers AI). Takes `{text, model}` (an **approved** model id), maps it → the Cloudflare model id via `getEmbeddingModel` (an allowlist — a caller can't steer the upstream path; unknown → **400**), forwards `{text}` to `…/ai/run/<cloudflareModel>`, and relays Cloudflare's `{result:{data}}`. (1) **Auth-gated** — `createClient()` → `getUser()`, 401 if signed out; (2) **injects the operator's** server-held `CF_ACCOUNT_ID` + `CF_API_TOKEN` (never `NEXT_PUBLIC_`; 503 if unset) so every signed-in player gets retrieval and the browser sends no key. The operator spends a free-tier quota, so it is also **per-user rate-limited**: after the auth gate it calls the `check_rate_limit` SECURITY DEFINER RPC (bucket `"embed"`) and returns **429 + Retry-After** when a user exceeds `EMBED_RATE_LIMIT_MAX` requests per `EMBED_RATE_LIMIT_WINDOW_SECONDS` (defaults 30/60s; `EMBED_RATE_LIMIT_MAX=0` disables). Fails **closed** (503) if the limit can't be verified; `retrieveLoreForTurn` swallows 400/429/503 best-effort, so a throttled turn just runs on curated lore. Logs upstream ≥400 status to Vercel logs; the token/request text are never logged. Used by the `cloudflare` embedding transport (`@/lib/ai` `embeddings.ts`) via `lore-retrieval-client.ts`. (Replaced an earlier `/api/proxy/ollama-cloud/api/embed` route: ollama.com hosts no embedding models, so its `/api/embed` 401s.)

## Testing

- **Route handlers** (`api/proxy/**/route.ts`, `auth/callback/route.ts`) are under the coverage gate and have colocated `route.test.ts` tests that invoke the exported `GET`/`POST` with a mocked `NextRequest` and a stubbed Supabase client — covering the auth gate, rate-limit allow/deny (429 + `Retry-After`, fail-closed 503), the model allowlist (400), upstream relay/diagnostics, and the never-log-the-key guarantee. Add cases here when you change a route's status codes or logging.
- **Middleware** (`src/proxy.ts`) is covered by `src/proxy.test.ts` (CSP nonce, `'strict-dynamic'`, dev-only `'unsafe-eval'`, the hardening headers, the matcher) and additionally asserted against the running server in `e2e/security-headers.spec.ts`.
- **Pages/layouts** are not unit-tested in jsdom; cover user-facing behaviour with an `e2e/` Playwright spec instead (auth forms, the redirect boundary, the PWA surface, real-browser a11y).
