...
- `gazetteer.ts` — Epoch- AND city-keyed map atlas (character epoch isolation, issue #101): `gazetteerForEpoch(epoch, cityId?)` returns, for the Fifth Epoch, the **current city's** districts (Tingen/Backlund/Trier/Bayam plus the start regions Pritz/Enmat/Feysac — each with its own public district list) with a header naming that city and "farther afield" travel to the OTHER Fifth cities; an unrecognised/absent `cityId` falls back to Tingen. Earlier epochs return their static, public region list (travel disabled — the CITIES model is Fifth-Epoch only). `uncertainFifthGazetteer()` (Backlund location sync) is the neutral atlas the map shows when a Fifth-Epoch city genuinely can't be resolved — no districts (so nothing is mis-pinned) but every city listed as travel-reachable, so setting out re-anchors `currentCity` and self-heals; the game-layer resolver `@/lib/game/location.ts` (`mapAtlasFor`) chooses it instead of defaulting to Tingen. The `cityId` is supplied by the caller (`cityIdFromLocation(location)` in `map-panel.tsx`) — the gazetteer must not import the game-layer city resolver. Player-safe public knowledge only; the narrator-only curated lore must not bleed in. Consumed by `map-panel.tsx`. (Movement reachability — `@/lib/game/place-graph.ts`, issue #101 — keys on `cityIdFromLocation`, NOT on these district keywords, so it never false-attributes one city's sub-location to another.)
