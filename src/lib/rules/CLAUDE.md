...
- `pathways.ts` — Pathway and sequence definitions (abilities, ingredients, rituals) for all 22 pathways. Large block of game data. At module load `applyCanonAdvancement` overlays the corpus-derived `ADVANCEMENT_RITUALS` (`advancement-canon.ts`) so `ALL_PATHWAYS` reflects canon: an **Advancement Ritual exists only from Sequence 5 onward** — higher rungs (Seq 9-6) carry `advancementRitual: undefined`, Seq 5-1 take the canon ritual text + material list (falling back to the hand-authored placeholder where the corpus lacked one). Then `applyCanonMainIngredient` grounds **every rung's single main ingredient** in the corpus (`main-ingredients-canon.ts`, generated from the same wiki `Module:Sequence/standard`): a rung the wiki documents a monster material for takes the canon **PRIMARY** material (e.g. Fool Seq 7 = "True Root of a Mist Treant", Red Priest Seq 7 = "Fire Salamander gland"; the rung's other canon materials ride along in the item description as alternatives), and a rung it documents none for takes the same-tier **role** Characteristic `"{seq.name} Beyonder Characteristic"` — the wiki's "Or a {role} Beyonder Characteristic" option. This replaces the hand-authored ingredient names, which had two corpus errors: the Seq 4-1 Characteristic was named **one tier too weak** (a Bizarro Sorcerer Seq-4 potion canonically takes a _Bizarro Sorcerer_ Characteristic, not the weaker Marionettist's — wiki-confirmed), and the low-rung monster materials were paraphrased or wrong (e.g. "Fog Tree Person Root" → canon "True Root of a Mist Treant"). Drift-proof: `main-ingredients-canon.test.ts` reconciles `pathways.ts` against the generated map (the `sequence-names-canon.test.ts` pattern). Then `applyCanonDemigodAbilities` (`demigod-abilities.ts`, issue #120) overlays the corpus-derived **demigod-rung abilities** (Seq 4-1 of pathways 10-22), replacing the old hand-authored placeholders. **Acting requirements are bespoke per rung (issue #95):** every sequence of all 22 pathways carries three sequence-specific acting requirements grounded in that rung's canon sequence name + the corpus-derived abilities — the old generic per-rung template for pathways 10-22 ("Live the role of a {name}, leaning into the pathway of {x}" + one pathway-wide theme) was replaced with role-specific lines that mirror the original nine's style and Saint/Angel tiering. Lookups: `getPathway`/`getSequence` (O(1) by id).
...
- `advancement-canon.ts` — **AUTO-GENERATED** (`pnpm rag:advancement-canon`, do not hand-edit). `ADVANCEMENT_RITUALS[pathwayId][level]` (levels 5-1) and `RITUAL_FROM_SEQUENCE`, extracted from the committed wiki dump's `Module:Sequence/standard` (`corpus/wiki/`). The single source of canon Advancement Ritual data. Each rung carries `description` + the flat `requirements` (the canon ingredient list) AND **tagged `steps`** (issue #209): each ingredient as a `{kind:"material"}` step plus the lived condition beats split from the description as `{kind:"condition"}` steps (capped at `MAX_CONDITION_BEATS`), so the game engine (`@/lib/game/ritual.ts`) can reconcile materials with the potion reagents while enforcing the conditions symbolically. `advancement-canon.test.ts` guards the tagging.
