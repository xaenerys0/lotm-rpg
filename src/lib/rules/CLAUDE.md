...
extrapolation). Their **acting requirements remain a generic per-rung template**
where the corpus gives no acting text, and the canon **Advancement Ritual** overlay
...
overlaid from `demigod-abilities.ts`, issue #120); only their acting
requirements remain a generic per-rung template where the corpus is silent. Classification scales with
