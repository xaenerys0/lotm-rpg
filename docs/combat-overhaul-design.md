...
1. **You fight the wrong people.** `deriveEncounterEnemy(state, ambush)`
   ...
   the combat target. There is no "who do you fight?" step and no framing for *why*.
...
| **Opponents** | **Do NOT gate allies out.** Instead, frame *why* an ally fights you (mind-controlled by another Beyonder, lost control, hidden motive, coerced, fighting against their will). Plus: story-designed encounters, explicit target choice, and richer (lore-grounded) enemy identity. |
...
### 4.1 Encounter framing — *who you fight, and why*
...
1. **Present NPCs** — each present NPC is offered *with* a framing. Allies are NOT
...
### 4.2 Combat ability kits — *pathway fantasy + tactical depth*
...
*names* as flavor labels on generic "ability" options. We promote them to a real toolkit.
...
### 4.3 The control meter — *loss of control as a live, escalating threat*
...
- **Legibility (clarity):** every option shows a plain-language effect tag and a coarse
  ...
  *direction and risk*, never the exact hidden number — preserving tension.

### 4.5 Outcome spectrum — *more than win/lose*

`CombatOutcome` expands from `victory | defeat | escape | stalemate` to add:

```ts
export type CombatOutcome =
  | "victory" | "defeat" | "escape" | "stalemate"   // existing
...
**Required change:** *artifacts persist unless their description says they're consumed.*
...
- `computeConsequences`, `scorePreparation` (ritual-material handling), and the dynamic
  ...
  cost (`artifactBacklash`) still applies on use — the *risk* of invoking a relic stays; it
...
4. **After-action report** (resolution): itemized — outcome, sanity delta (with cause),
   injuries (severity + recovery), items gained/lost (and explicitly *what was NOT*
...
- *Outcome:* compiles, all existing tests pass, no gameplay change.
...
- **Scope creep into a full round-based system.** Explicitly out of scope — the owner chose
  to *deepen* the 3-phase model. Round-based remains a possible future.
...
- **Option Q3 — Player-driven/freeform (variable).** Lean on the AI narrator with engine
  guardrails; ingredient acquisition stays engine-gated but the *path* is open free-text.
